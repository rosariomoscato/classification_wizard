  ...
  if status == True:
    ...
    with col2:
      test_size = st.number_input('Please enter test size', 0.01, 0.99, 0.25,
                                  0.05)
      number_neighbors = st.number_input('Please enter number of neighbors',
                                         value=5,
                                         step=1)
    ...
    with st.expander('Advanced Parameters'):
      ...
      with col2_1:
        ...
        metric = st.selectbox('Distance metric', [
          'minkowski', 'euclidean', 'manhattan', 'chebyshev', 'wminkowski',
      ...
      st.markdown(
        'For further information please refer to ths [link](https://scikit-learn.org/stable/modules/generated/sklearn.neighbors.KNeighborsClassifier.html)'
      )
    ...
    try:
      ...
      if status_test == True:
        try:
          ...
          st.markdown(download(
            X_pred,
            'Classification Wizard - K-Nearest Neighbors Classifier - Predicted Labels'
          ),
      ...
      st.warning(
        'Please select at least one feature, a suitable label and appropriate parameters'
      )
